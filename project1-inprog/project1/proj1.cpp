  ...
  const std::string& optarg )
{
  ...
  // Otherwie catch an out_of_range exception throw logic error
  try
  {
  ...
  catch( const std::out_of_range& )
  {
    ...
    throw std::logic_error{"Mistakes were Made"};
  }
  ...
  const std::string& optarg )
{
  ...
  if( tmp > UINT16_MAX )
  {
    ...
    throw std::logic_error{"Overflow Number"};
  }
...
// Overloading << to automatically convert images to strings
std::ostream& myImage::operator<<(
  std::ostream& s,
  Image& img )
{
  return s << img.toString();
}
